...
internal class GraphTest {
    ...
    inner class AdjacentListGraphTest {
        private val g = AdjacentListGraph(
            MutableList(5) { Vertex<Int, Int>(it) }
        )
        ...
        fun testGetEdges() = listOf(
            ...
            DynamicTest.dynamicTest("edges from $input should be $expected") {
                assertEquals(expected, g.edgesFrom(input).map { it.to })
            }
        }
    ...
    @Nested
    inner class AdjacencyMatrixGraphTest {
        private val g = AdjacencyMatrixGraph<Int, Int>(
            Array(5) { Array<Int?>(5) { null } }
        )
